...
public class FindShadows implements Runnable, IConst {
    ...
    private LinkedList<int[]> searchQuery = new LinkedList<>();
    /*name opened file*/
        ...
        if (one > zero) {
            ...
            for (int row = 0; row < binaryArray.length; row++)
                for (int col = 0; col < binaryArray[row].length; col++) {
                    if (binaryArray[row][col] == curSilhouettes) { //todo ternar op
    ...
    private void findShadows(int row, int col) {
        ...
        while (!searchQuery.isEmpty()) {
            /* extract the coordinates of the cell hat is first in the queue*/
            coordinatesFirstFindCell = searchQuery.getFirst();
            row = coordinatesFirstFindCell[0];
            col = coordinatesFirstFindCell[1];

            /* del from queue first arr*/
            searchQuery.removeFirst();
