package com.shpp.p2p.cs.dgrymach.assignment3.lec08;
...
public class Ukraine1 extends WindowProgram {
    ...
    public void run() {
        try {
            ...
            while (true) {
				 ...
				 * Если любая из вышеперечисленных трех переменных имеет значение null, то Долгота
                   будет null, потому что его вызов readLine не будет иметь никаких данных.
				 */
            ...
            println("How unpatriotic."); //Как непатриотично
        }
    }

    /**
     * Given the longitude and latitude of a city (in string form), displays
     * a dot for that city.    Учитывая долготу и широту города (в виде строки), отображает точка для этого города
     *
    ...
    private void plotOneCity(String cityName, String cityLat, String cityLong) {
        ...
        //Определите, где на экране должен быть нарисован город
    ...
    /**
     * Given a raw longitude, returns the screen x coordinate where     необработанный
